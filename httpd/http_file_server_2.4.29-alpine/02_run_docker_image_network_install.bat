...
REM docker network create docker-build
docker run --rm --network docker-build --hostname=%CONTAINER_NAME% -v "%SCRIPT_PATH%\src\conf:/usr/local/apache2/conf" -v "%SCRIPT_PATH%\src\www:/usr/local/apache2/htdocs" --name %CONTAINER_NAME% --detach -p %LOCAL_PORT%:%REMOTE_PORT% %IMAGE_NAME%
