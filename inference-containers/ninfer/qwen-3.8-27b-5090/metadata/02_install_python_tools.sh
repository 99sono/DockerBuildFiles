...
#
# NOTE: parse_docker_log.py is PURE STDLIB (re / statistics / argparse /
# datetime / math) — no pip packages are required, which keeps the env
# 100% reproducible with zero network dependencies. This script only
# proves the env has a working python and that every import the parser
# uses resolves.
#
