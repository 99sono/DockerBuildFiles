...
|-----------|-------|---------|
| `--max-model-len` | 262144 | Maximum context window (~256K tokens) |
| `--max-num-batched-tokens` | 65536 | Prefill batch size (Godzilla mode for DGX Spark) |
| `--gpu-memory-utilization` | 0.65 | 65% of VRAM reserved for model + KV cache (DGX Spark stability) |
| `--max-num-seqs` | 8 | Multi-user parallelism |
| `--kv-cache-dtype` | fp8_e4m3 | FP8 KV cache for memory efficiency |
| `--moe-backend` | flashinfer_cutlass | Fast MoE kernel fusion |

### Context Length Limits

With this configuration:
- **Max total tokens:** 262,144 (~256K tokens, prompt + response combined)
- **Available KV cache:** Leverages 128GB unified memory architecture
- **Example:** You can paste a 200K token document and receive a ~50K token response
...
| Batch Size | Prefill Speed | KV Cache Room | 256K Context? |
|------------|---------------|---------------|---------------|
| **65536** | **Fastest (Godzilla mode)** | **~190K+ tokens** | ✅ **Yes (recommended)** |
| 32768 | Fast | ~220K+ tokens | ✅ Yes |
| 8192 | Conservative | ~250K+ tokens | ✅ Yes |
...
2. Lower `--max-num-batched-tokens` to 32768
3. Reduce `--gpu-memory-utilization` to 0.55
...
If enabling `VLLM_USE_V2_MODEL_RUNNER` causes crashes, keep it commented out. The V2 runner (async CPU/GPU overlap) requires additional memory during initialization and may not be compatible with 256K context on DGX Spark's unified memory architecture.
