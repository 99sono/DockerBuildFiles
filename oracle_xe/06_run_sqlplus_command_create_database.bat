...
docker exec -it %CONTAINER_NAME%  /bin/bash -c "echo -e 'CREATE USER %DB_USER_NAME% IDENTIFIED BY %DB_USER_NAME% DEFAULT TABLESPACE USERS;' | sqlplus SYS/oracle as SYSDBA"

ECHO GOING TO ADD PERMISSIONS
docker exec -it %CONTAINER_NAME%  /bin/bash -c "echo -e 'grant CREATE SESSION, ALTER SESSION, CREATE DATABASE LINK, CREATE MATERIALIZED VIEW, CREATE PROCEDURE, CREATE PUBLIC SYNONYM, CREATE ROLE, CREATE SEQUENCE, CREATE SYNONYM, CREATE TABLE, CREATE TRIGGER, CREATE TYPE, CREATE VIEW, UNLIMITED TABLESPACE TO %DB_USER_NAME%;' | sqlplus SYS/oracle as SYSDBA"
